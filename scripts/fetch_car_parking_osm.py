...
data/parking/public/garages.json or public/lots.json (City visitor map) is skipped,
so the app prefers official names/pricing over near-duplicate OSM pins.
...
def load_official_arcgis_latlngs() -> list[tuple[float, float]]:
    """City visitor-map garage/lot centroids (same source as fetch_car_parking_arcgis.py)."""
    out: list[tuple[float, float]] = []
    for path in (PUBLIC_GARAGES_JSON, PUBLIC_LOTS_JSON):
        if not path.is_file():
            continue
        try:
...
def main() -> int:
    ...
    official_pts = load_official_arcgis_latlngs()
    ...
    for el in data.get("elements") or []:
        ...
        if not item or not item_within_gr_center(item):
            continue
        if official_pts and item_near_official_parking(item, official_pts):
            continue
    ...
    note_common = (
        ...
        "Excludes features within ~0.06 mi of any City (ArcGIS) garage/lot centroid. "
