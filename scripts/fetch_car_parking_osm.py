...
def load_official_arcgis_latlngs_by_kind() -> tuple[
    list[tuple[float, float]], list[tuple[float, float]]
]:
    """City visitor-map centroids: (public garages, public lots) separately."""
    garages: list[tuple[float, float]] = []
    lots: list[tuple[float, float]] = []
    for path, bucket in (
        (PUBLIC_GARAGES_JSON, garages),
        (PUBLIC_LOTS_JSON, lots),
    ):
        if not path.is_file():
            continue
        try:
        ...
        for item in doc.get("items") or []:
            ...
            if lat is None or lon is None:
                continue
            try:
                bucket.append((float(lat), float(lon)))
...
def main() -> int:
    ...
    for el in data.get("elements") or []:
        ...
        if parking_osm_bucket(tags) == "garage":
            if official_garages_pts and item_near_official_parking(
